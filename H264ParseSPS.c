...
INT h264_parse_sps(const BYTE *data, UINT dataSize, sps_info_struct *info)
{
    ...
    cout<<"init index: "<<bs.index<<endl<<" "<<" data size "<<dataSize<<endl<<hex<<(unsigned int) (unsigned char)(bs.data[bs.index])<<endl;;
    
    int i=0;
    for(;i<dataSize;i++)
    {

    cout<<" "<<hex<<(unsigned int) (unsigned char)(bs.data[i])<<dec;
    }
    cout<<endl;
    ...
    cout<<"get nalu type index: "<<bs.index<<endl<<" "<<hex<<(unsigned int) (unsigned char)(bs.data[bs.index])<<endl<<dec;
 
    cout<<"type: " <<nal_unit_type<<endl;
    if (nal_unit_type == 0x7) {     //Nal SPS Flag
        info->profile_idc = u(&bs, 8);
     
        cout<<"profile_idc: "<<info->profile_idc<<endl;
        ...
        cout<<"level_idc:  "<<info->level_idc<<endl;
	...
	cout<<"poc: "<<pic_order_cnt_type<<endl;

        if (pic_order_cnt_type == 0) {
            UINT max_frame=ue(&bs);        //log2_max_pic_order_cnt_lsb_minus4

	    cout<<"log_2_max_frame_minus_4: "<<max_frame<<endl;
       ...
       cout<<"begin read width and height index:"<<bs.index<<endl<<" "<<hex<<(unsigned int) (unsigned char)(bs.data[bs.index])<<endl<<dec;

	UINT pic_width_in_mbs_minus1 = ue(&bs);     //第36位开始
           
	cout<<"begin read height index: "<<bs.index<<endl;

	UINT pic_height_in_map_units_minus1 = ue(&bs);      //47
  
        cout<<"begin read mbs_flag index: "<<bs.index<<endl;

	UINT frame_mbs_only_flag = u(&bs, 1);
        
	cout<<"width "<<pic_width_in_mbs_minus1<<" hei "<<pic_height_in_map_units_minus1<<endl;
    ...
    cout<<"real width "<<info->width<<" real height "<<info->height<<endl;
    return ret;
}
